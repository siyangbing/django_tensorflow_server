...
resize_shape = 0.5
# 原始img文件夹路径
img_path = "/home/db/文档/red_螺丝钉"
saved_img_path = "/home/db/文档/red_螺丝钉_result"
# 裁剪的大小
crop_size = (640, 640)
# 重合的边界
border = 110
...
# saved_model_dir = "/home/sucom/Documents/5720_ld_20201013_all/model/frozen_58179/saved_model"
saved_model_dir = "/home/db/dbing/models-master/research/object_detection/ztuiduan/new_models/red_lsd/saved_model"
...
class CropImageLabel():
    ...
    def tuiduan_img(self, img, sess1):
        ...
        # input_data = np.concatenate((input_data,input_data,input_data,input_data,input_data,input_data,input_data,input_data,input_data,input_data,input_data),axis=0)
        ...
        print("eval time:{}".format(time.time()-t0))
            ...
            for y_l in range(w_num):
                ...
                if points_list:
                    for point in points_list:
                        ...
                        pingjie_points_list.append([px_min, py_min, px_max, py_max, point[4],point[5]])
    ...
    with sess1:
        ...
        for root, folders, files in os.walk(img_path):
            ...
            for file in files:
                ...
                cv2.imwrite(saved_img_path + "/{}".format(img_path_one.split("/")[-1]), result_img)
                print("saved {}".format(saved_img_path + "/{}".format(img_path_one.split("/")[-1]), result_img))
                t1 = time.time()
